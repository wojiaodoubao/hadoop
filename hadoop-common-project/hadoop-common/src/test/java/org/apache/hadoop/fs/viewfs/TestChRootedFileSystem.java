...
public class TestChRootedFileSystem {
  ...
  static FileSystem getChildFileSystem(ViewFileSystem viewFs, URI uri) {
    for (FileSystem fs : viewFs.getChildFileSystems()) {
      if (fs.getUri().getScheme() != null && fs.getUri().getScheme()
          .equals(uri.getScheme()) || uri.getScheme() == null) {
        if (fs.getUri().getAuthority() != null && fs.getUri().getAuthority()
            .equals(uri.getAuthority()) || uri.getAuthority() == null) {
          return fs;
        }
      }
    }
